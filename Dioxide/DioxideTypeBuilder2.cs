...
namespace Dioxide
{
    public class DioxideTypeBuilder2
    {
        private readonly IGeneratorDiagnostics _diagnostics;

        private readonly Dictionary<Type, TypeConfigurator> _hash;

        public DioxideTypeBuilder2(IGeneratorDiagnostics diagnostics)
        {
            _hash = new Dictionary<Type, TypeConfigurator>();
            _diagnostics = diagnostics;
        }

        public DioxideTypeBuilder2 GenerateDecorator<TInterface>(Action<IDioxideTypeConfigurator<IVisitor>> configurationAction)
        {
        ...
        public DioxideTypeBuilder2 GenerateDecorator<TInterface>()
        {
            var configurator = GetOrCreateTypeConfigurator<TInterface>();
            configurator.AppendGenerateType(GenerateType.Decorator);
